#!/usr/bin/env python3
"""
perf_baseline.py — measure API latency for an HTTP API's hot endpoints.

Captures per-endpoint: cold + warm p50/p95, body size, the four caching
headers (ETag, Cache-Control, Content-Encoding, X-Cache), and a
conditional-GET probe (If-None-Match with the cold ETag) that proves
whether the server's ETag is real or decorative.

Stdlib only — no requests/httpx dependency. Writes results to
/tmp/perf_<label>.json so before/after diffs work.

Usage:
    python perf_baseline.py [BASE_URL] [TOKEN] [LABEL]
    python perf_baseline.py https://prod-url eyJ...
    PERF_BASE_URL=... PERF_TOKEN=... python perf_baseline.py

Defaults:
    BASE_URL = https://distinguished-energy-production-e3d0.up.railway.app
    TOKEN    = (read from $PERF_TOKEN env var)
    LABEL    = (auto-derived from BASE_URL host, or argv[3])
    OUT      = /tmp/perf_<label>.json
    N_WARM   = 10
"""
...
# ─── Endpoints (edit per audit) ──────────────────────────────────────────────
# Convention: ("METHOD", "/path?query=...") tuples. For a frontend-driven
# audit, derive this list by grepping ui/js/views/* for `api('/api...')`
# calls — don't guess which endpoints matter.
...
N_WARM = 10  # warm calls per endpoint after the 1 cold call


# ─── Internals ───────────────────────────────────────────────────────────────


def _request(url: str, method: str, token: str, extra_headers: dict[str, str] | None = None) -> tuple[float, int, int, dict[str, str]]:
...
def main(argv: list[str]) -> int:
    ...
    for method, path in ENDPOINTS:
        ...
        cold_etag = cold_headers.get("ETag")
        cold_cache = cold_headers.get("Cache-Control")
        cold_enc = cold_headers.get("Content-Encoding")
        cold_x_cache = cold_headers.get("X-Cache")
        ...
        for _ in range(N_WARM):
            t, s, sz, h = _request(url, method, token)
            warm_times.append(t)
            warm_statuses.append(s)
            warm_sizes.append(sz)
            last_headers = h

        # Conditional GET probe: send If-None-Match with cold ETag
        cond_status = cond_size = None
        if cold_etag:
            t, cond_status, cond_size, _ = _request(
                url, "GET", token, extra_headers={"If-None-Match": cold_etag}
            )
            _ = t  # not used for percentiles
