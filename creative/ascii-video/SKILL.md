...
## ⚠️ SUPERSEDED for marketing videos → use `hyperframes` skill

For marketing videos, demos, social content, and product tours on this project, the **HyperFrames** skill (`creative/hyperframes`) has replaced this raw PIL pipeline. HyperFrames produces HTML+GSAP motion graphics with proper typography, scene transitions, and polished output — matching the Harbor design system.

**When to still use this skill:** raw creative ASCII art, generative visualizers, experimental/creative-only tasks, or when the user explicitly asks for a terminal aesthetic.

**When to use `hyperframes` instead:** product demos, social media shorts, animated title cards, explainer videos, narrated product tours, motion graphics — any task where the output should look polished and modern rather than retro/terminal.
...
### TTS Narration Sync — Always Match Scene Duration to Actual Audio

When generating narrated videos (TTS + generative ASCII), **do NOT hardcode scene durations**. The actual TTS audio duration must be measured and used as the scene duration. Without this, narration will be cut off at the end:

```bash
# Measure actual audio duration
ffprobe -v error -show_entries format=duration -of csv=p=0 narration.ogg
```

If the rendered clip is shorter than the TTS audio, pad with frozen last frame using ffmpeg's tpad filter:
```bash
pad=$(echo "$tts_dur - $clip_dur" | bc -l)
ffmpeg -i clip.mp4 -vf "tpad=stop_mode=clone:stop_duration=$pad" padded.mp4
```

Then mux padded video with full audio:
```bash
ffmpeg -i padded.mp4 -i narration.ogg -c:v copy -c:a aac -shortest output.mp4
```

### Social Media Video Structure — Shorts Over Compilations

For social media marketing (Buffer, Instagram, Facebook), produce **individual short videos** (20-35s each, one per use case) rather than one long compilation (3+ min). Reasons:
- Social platforms limit feed videos to 60s (Instagram) or have lower engagement for long content (Facebook).
- Each short can be posted individually over time, creating a content series.
- Individual shorts are schedulable via Buffer's queue.
- A 20-35s video stays under Instagram's 60s feed limit.

When asked for "videos for social media," default to 10-30 second shorts, one per concept/use case, unless the user explicitly says "compilation."

### Dual-Format Rendering (Landscape + Portrait)

For cross-platform social media posting, render each scene in TWO formats:

| Format | Dimensions | Platform | Use |
|--------|-----------|----------|-----|
| Landscape | 1920×1080 (16:9) | Facebook feed | Long-form copy, stat counters |
| Portrait | 1080×1920 (9:16) | Instagram feed/stories | Short-form copy, mobile-native |

The render script should accept a `--mode landscape|portrait|both` argument. Render both by default. Keep a single source of truth for scene functions — only the grid dimensions differ between formats.

For Instagram, all videos must be **under 60 seconds** (Instagram feed video limit). Trim the portrait version if needed:
```bash
ffmpeg -i portrait_full.mp4 -t 59 -c copy portrait_59s.mp4
```

### Narration Copy Must Be Factually Accurate

When writing TTS narration for marketing videos about a product, **verify all feature claims against the actual product documentation**. Do NOT invent features, stats, or capabilities. Specific pitfalls from this session:

- **Do not invent features the product doesn't have**. Verify against docs/features/*.md before writing any copy. If a feature doesn't exist in the docs, don't claim it.
- **Do not use technical jargon** in the narration. The underlying tech stack (DeepSeek, cache, tri-stage pipeline, prompt synthesis) is invisible to users. Write for the customer's audience. "AI research assistant" not "DeepSeek synthesis." "Data-backed" not "prompt-cached."
- **Stats must be real**. Query the actual database or read the project's STATE-SNAPSHOT.md for entity counts. Do not fabricate numbers. Wrong numbers will be corrected by the domain expert.
- **Buffer API video posting** uses `assets: { videos: [{ url: ... }] }` in the `CreatePostInput`, not `media:` or `attachments:`. The video URL must be publicly accessible (hosted on a running Railway service or public CDN).
- **Delete existing posts before scheduling new ones**: fetch all posts with `status: 'scheduled'`, delete via `deletePost` mutation, then schedule new ones. Buffer's queue is first-in-first-out.
- **Scene index starts at 0 for file naming** (`clip_00.mp4` = scene 1). Keep scene numbers consistent between file names and TTS file names.
