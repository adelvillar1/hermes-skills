...
metadata:
  hermes:
    tags: [Design, Research, Skills, Knowledge, Curator]
    related_skills: [design-motion-principles, ui-implementation-review, animation-vocabulary, apple-design, design-spec-agent-prompts, expert-skill-builder]
---
...
- Before creating anything: `skills_list` + `skill_view` the likely umbrellas (design-motion-principles, design-engineering, ui-implementation-review, animation-vocabulary, apple-design, tech-stack-evaluation, sketch…).
...
- `mnemosyne_remember` (importance ~0.7): URL, what it's good for, which skill + section absorbed it, license.
...
- Scope it as a plan (draft-feature-plan) → dispatch implementer (subagent-driven-development) → 2-stage review → verify → commit on `develop` (DesignCanvas flow: develop → main, never commit on main).
