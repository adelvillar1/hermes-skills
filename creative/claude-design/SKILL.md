...
## Rule 0: Discovery Form (Do NOT skip)

**Do not write any code before completing discovery.** Before touching a file, batch all questions in a single message. Maximum 6 questions.

Pick the right subset:

| Question | When |
|----------|------|
| What are you designing? | Always |
| Who's the audience? | Always |
| What's the brand/product? | Always — determines if we need real brand assets |
| What tone? | If not obvious |
| What format? (web, deck, mobile, print) | If not clear from context |
| Any existing design system or references? | If brand is named |
| The one thing it must communicate? | Always — prevents scope creep |
| Any constraints? | Optional |

**Example (single message, do not drag into multiple turns):**

```
Before I write anything:
1. What are you designing? (landing page, UI mockup, deck, etc.)
2. What's the brand or product?
...
Wait for the user's response before writing code. If the user says "just go" after questions, proceed with explicit assumptions labeled.
...
- inspect screenshots at the primary viewport (save screenshot to disk if `browser_vision` fails; use `vision_analyze` with file path, or note the limitation and proceed)
