...
## Step 6: Runtime State Verification (when applicable)
- [ ] Query live database for row counts, top/bottom samples, stale detection
- [ ] Check Redis cache keys and TTLs for the domain
- [ ] Verify API endpoint health with a curl or fetch call
- [ ] Check pipeline job status / last run timestamps
- [ ] Note discrepancies between docs and live state (e.g., "doc says 966 GLBs" but table was dropped)
...
- [ ] **Live state summary** — current counts, cache coverage, known gaps (table format)
- [ ] **Potential issues / gaps** — flagged with severity (🔶 medium, 🟡 low)
...
- [ ] Live state matches documented expectations (or discrepancies are explicitly noted)
