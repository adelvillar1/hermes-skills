...
- Give it a name like "the harness-agent"
- Select scopes:
...
- Give it a title like "the harness-agent-<machine-name>"
...
uv run python "${SKILLS_HOME:-$SKILLS_HOME}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _hermes_env="${SKILLS_HOME:-$SKILLS_HOME}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hermes_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${SKILLS_HOME:-$SKILLS_HOME}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
