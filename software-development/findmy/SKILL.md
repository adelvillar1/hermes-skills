...
Then use `your vision tool` to read the screenshot:
```
your vision tool(image_url="/tmp/findmy.png", question="What devices/items are shown and what are their locations?")
```
...
```
your vision tool(image_url="/tmp/findmy-detail.png", question="What is the location shown for this device/item? Include address and coordinates if visible.")
```
...
2. Use `your vision tool` to read screenshot content — don't try to parse pixels
3. For ongoing tracking, use a your scheduler tool to periodically capture and log locations
