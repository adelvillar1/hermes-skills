...
`$SKILLS_HOME/harness config file` (defaults to `your agent config dir config file` when `SKILLS_HOME` is unset)
...
Restart the the harness session for changes to take effect. The MCP connection is
