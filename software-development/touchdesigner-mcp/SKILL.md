...
```
the harness -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SKILLS_HOME:-$SKILLS_HOME}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to the harness config (if missing)
...
3. **Restart the harness session** to pick up the new MCP server
