...
HERMES_HOME_DIR="${SKILLS_HOME:-$SKILLS_HOME}"
HERMES_CFG="${HERMES_HOME_DIR}/harness config file"
MCP_PORT=40404
...
# ── 3. Ensure the harness config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} the harness config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in the harness config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to the harness config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart the harness session to pick up config change")
fi
