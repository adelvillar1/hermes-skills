...
- Load this skill: `skill loader (name="hyperframes-expert")`.
...
- **Do not use the `hyperframes` authoring skill's bundled skills verbatim.** Those slash commands target Claude Code, Cursor, etc. -- the user is asking you to be an expert via the harness tools, not to register slash commands. Translate the same knowledge into `terminal`/`read_file`/`write_file`/`web_extract` invocations.
...
- The skill loaded with no parse errors: `skill loader (name="hyperframes-expert")` returns the full SKILL.md.
