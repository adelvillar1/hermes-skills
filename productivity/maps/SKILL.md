...
- **Node.js fetch requires User-Agent header.** Without it, Overpass returns HTTP 406.
  The query must be sent as `body: 'data=' + encodeURIComponent(query)` with
  `Content-Type: application/x-www-form-urlencoded`. See `references/overpass-api-nodejs.md`
  for the full working pattern and error handling.
- Overpass returns `lon` (not `lng`) in geometry point objects. Always handle both
  fields (`pt.lon ?? pt.lng`) when working with building/road geometry.
