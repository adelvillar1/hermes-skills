...
- **Pin routines to stable model aliases, never versioned names.** Providers retire versioned names
  and silently re-route them (DeepSeek 2026-09-14: `deepseek-v4-flash` / `-vision-exp` retired and
  routed to the new Flash at new pricing; the live `/v1/models` registry lists exactly
  `deepseek-flash` and `deepseek-v4-pro`). A versioned-name pin keeps a stale `model_snapshot` and
  then fails closed on global model changes; an alias (`deepseek-flash`) tracks the vendor's current
  model with zero maintenance. Audit pins with `hermes cron list` per profile.
