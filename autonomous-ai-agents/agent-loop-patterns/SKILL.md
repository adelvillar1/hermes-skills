...
- **An auditor must mirror the pruned tree, not the raw filesystem**: when a checker re-implements another system's lookup, replicate that system's exclusion rules too (hidden/archive/vendor dirs, support folders) — walking directories the real resolver prunes reports collisions that cannot occur and inflates counts that then get repeated as fact. Diff your candidate set against one known-good lookup before publishing a number.
