...
- **A journal's date is not its coverage**: infer coverage from artifacts, not filenames — a session that appends to an older-dated journal file, or ships its journal inside a code commit, *is* covered, while a docs-only commit that adds a journal entry for an earlier session is not. Every heuristic here has both false positives (a covered session reported as a gap) and false negatives (a real gap masked by a neighbouring entry), so sweep the detector against days known to be covered and days known to be uncovered before trusting its output.
- **An auditor must mirror the pruned tree and read the artifact**: when a checker re-implements another system's lookup, replicate that system's exclusion rules too (hidden/archive/vendor dirs, support folders) — walking directories the real resolver prunes reports collisions that cannot occur and inflates counts that then get repeated as fact. Likewise, verify the artifact the subject *produced* (did this commit touch the evidence directory?) rather than guessing its name from prose: a bundle named for its feature can never match a slug derived from a commit subject, so the checker reports "no evidence" for work that shipped evidence. Diff your candidate set against one known-good lookup before publishing a number.
...
- **A review loop must converge, not repeat**: when the same finding survives N runs, stop re-narrating it and keep ONE always-current decisions file (item, evidence path, exact action, risk) that every run cites and updates, pruning entries as they resolve. Then each run reports only *changes* — new items, resolved items, newly blocked items. Order findings newest-first and let the cap fall on the oldest: a display cap sorted oldest-first (or an item list that only grows) hides the freshest, most actionable finding behind stale ones, which is how a live gap stayed invisible for days while the report kept printing the same three lines. An item list that still carries a problem the world already fixed is worse than no list: it trains the reader to skim and it hides real drift (re-verify every entry against the live system before repeating it).
