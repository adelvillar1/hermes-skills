...
metadata:
  hermes:
    tags: [project-management, project-init, methodology, retrofit, documentation]
    related_skills: [init-project-structure, slim-claude-md, draft-feature-plan, session-warmup, session-wrapup]
---
...
|-------|----------|------------|
| Methodology enforcement in hard rules | `delegate_task` + 2-stage review rule | Add to hard rules |
...
When writing multiple files in a batch, a typo in the path creates a file in a wrong directory (e.g., `riderscout-saas-maas-migrated` instead of `riderscout-saas-migrated`). Always verify with `ls` after writing, and clean up the typo directory immediately.
